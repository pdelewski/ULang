...
function getCharAt(message, scrollOffset, position) {
  let msgLen = len(message);
  if (msgLen == 0) {
    return 32;
  }
  let idx = (scrollOffset + position) % msgLen;
  return int(message.charCodeAt(idx));
}

function createScrollingTextProgram(message, scrollOffset, row) {
  let lines = [];
  let col = 0;
  while (true) {
    if (col >= TextCols) {
    ...
    let charCode = getCharAt(message, scrollOffset, col);
    let memAddr = TextScreenBase + row * TextCols + col;
...
function createMultiLineTextProgram(messages, scrollOffsets) {
  let lines = [];
  let row = 0;
  while (true) {
    if (row >= TextRows) {
...
function main() {
  ...
  let w = graphics.CreateWindow(
    "MOS 6502 Scrolling Text",
    windowWidth,
    windowHeight,
  );
  ...
  let frameCount = 0;
  let textColor = graphics.NewColor(0, 255, 0, 255);
  let bgColor = graphics.NewColor(0, 0, 0, 255);
  graphics.RunLoop(w, function (w) {
    ...
    let program = createMultiLineTextProgram(messages, scrollOffsets);
