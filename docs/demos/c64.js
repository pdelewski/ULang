...
const basic = {
  ...
  genPrint: function (args, cursorRow, cursorCol, ctx) {
    ...
    let scrollLabelNum = this.intToString(ctx.LabelCounter);
    lines = append(lines, "LDA $30");
  ...
  genExpression: function (expr) {
    ...
    } else if (expr.Type == this.ExprPeek) {
      lines = append(lines, "LDA " + this.toHex(expr.PeekAddr));
  ...
  GenScrollRoutine: function () {
    let lines = [];
    lines = append(lines, "JMP AFTER_SCROLL");
  ...
  parsePeekAddr: function (args) {
    ...
    while (true) {
      ...
      peekStr = peekStr + this.charToString(ch);
      i = i + 1;
    }
    if (peekStr != "PEEK") {
      return -1;
    ...
    if (i >= len(args) || args.charCodeAt(i) != 40) {
      return -1;
    }
    i = i + 1;
    while (true) {
    ...
    while (true) {
      ...
      addrStr = addrStr + this.charToString(int(ch));
      i = i + 1;
    }
    return this.parseNumber(addrStr);
  },
  ParseExpression: function (args) {
    args = this.trimSpacesStr(args);
    let peekAddr = this.parsePeekAddr(args);
    if (peekAddr >= 0) {
      return {
        Type: this.ExprPeek,
        PeekAddr: peekAddr,
...
function scrollScreenUp(c) {
  let row = 0;
  while (true) {
    if (row >= TextRows - 1) {
      break;
    }
    let srcAddr = TextScreenBase + (row + 1) * TextCols;
    let dstAddr = TextScreenBase + row * TextCols;
    let col = 0;
    while (true) {
      if (col >= TextCols) {
        break;
  ...
  let lastRowAddr = TextScreenBase + (TextRows - 1) * TextCols;
  let clearCol = 0;
  while (true) {
    if (clearCol >= TextCols) {
      break;
    }
    c.Memory[lastRowAddr + clearCol] = 32;
    clearCol = clearCol + 1;
  }
  return c;
}
