...
function makeLdaImm(value) {
  return "LDA #$" + toHex2(value);
}

function makeStaAbs(addr) {
  return "STA $" + toHex4(addr);
}

function createDrawRectProgram(startX, startY, endX, endY, color) {
  let lines = [];
  let row = startY;
  while (true) {
    if (row > endY) {
...
function createDiagonalLineProgram(x1, y1, x2, y2, color) {
  let lines = [];
  let steps = x2 - x1;
  if (steps < 0) {
    steps = -steps;
  ...
  while (true) {
    ...
    let x = x1 + (((i * (x2 - x1)) / steps) | 0);
    let y = y1 + (((i * (y2 - y1)) / steps) | 0);
    if (x >= 0) {
      if (x < 32) {
        if (y >= 0) {
          if (y < 32) {
            let addr = 0x0200 + y * 32 + x;
            lines = append(lines, makeLdaImm(int(color)));
            lines = append(lines, makeStaAbs(addr));
          }
        }
...
function main() {
  let scale = int32(16);
  let windowWidth = int32(cpu.ScreenWidth) * scale;
  let windowHeight = int32(cpu.ScreenHeight) * scale;
  let w = graphics.CreateWindow("MOS 6502 Emulator", windowWidth, windowHeight);
  let c = cpu.NewCPU();
  let program = [];
  let asmDemo = createSimpleDemo();
  let idx = 0;
  while (true) {
    if (idx >= len(asmDemo)) {
      break;
  ...
  if (len(program) > 0) {
    program = program.slice(0, len(program) - 1);
  }
  let rectProg = createDrawRectProgram(8, 4, 24, 12, uint8(2));
  idx = 0;
  while (true) {
    if (idx >= len(rectProg)) {
      break;
  ...
  let rectProg2 = createDrawRectProgram(4, 16, 12, 28, uint8(3));
  idx = 0;
  while (true) {
  ...
  let rectProg3 = createDrawRectProgram(18, 18, 28, 26, uint8(4));
  idx = 0;
  while (true) {
  ...
  let lineProg = createDiagonalLineProgram(2, 2, 30, 14, uint8(5));
  idx = 0;
  while (true) {
    if (idx >= len(lineProg)) {
      break;
  ...
  let lineProg2 = createDiagonalLineProgram(30, 2, 2, 14, uint8(1));
  idx = 0;
  while (true) {
  ...
  graphics.RunLoop(w, function (w) {
    ...
    while (true) {
      ...
      while (true) {
        ...
        let colorVal = cpu.GetScreenPixel(c, x, y);
        if (colorVal != 0) {
          let px = int32(x) * scale;
          let py = int32(y) * scale;
          if (colorVal == 1) {
            graphics.FillRect(
              w,
              graphics.NewRect(px, py, scale, scale),
              graphics.White(),
            );
          } else if (colorVal == 2) {
            graphics.FillRect(
            ...
            graphics.FillRect(
              w,
              graphics.NewRect(px, py, scale, scale),
              graphics.NewColor(255, 255, 0, 255),
            );
          } else if (colorVal == 6) {
            graphics.FillRect(
              w,
            ...
            graphics.FillRect(
              w,
              graphics.NewRect(px, py, scale, scale),
              graphics.NewColor(0, 255, 255, 255),
            );
          } else if (colorVal == 8) {
            graphics.FillRect(
              w,
              graphics.NewRect(px, py, scale, scale),
              graphics.NewColor(128, 128, 128, 255),
            );
          } else if (colorVal == 9) {
            graphics.FillRect(
              w,
              graphics.NewRect(px, py, scale, scale),
              graphics.NewColor(255, 128, 0, 255),
            );
          } else {
            graphics.FillRect(
              w,
