...
i = (i + 1);
    ...
    return {A: 0, X: 0, Y: 0, SP: 0xFF, PC: 0x0600, Status: 0x20, Memory: mem, Halted: false, Cycles: 0};
}
,
...
let overflow = ((((((c.A ^ value)) & 0x80)) == 0) && (((((c.A ^ uint8(result))) & 0x80)) != 0));
...
i = (i + 1);
        ...
        if ((b == 10)) {
tokens = append(tokens, {Type: this.TokenTypeNewline, Representation: [b]});
i = (i + 1);
            continue;
        }
        if ((b == 59)) {
...
i = (i + 1);
        ...
        if ((b == 35)) {
tokens = append(tokens, {Type: this.TokenTypeHash, Representation: [b]});
i = (i + 1);
            continue;
        }
        if ((b == 36)) {
tokens = append(tokens, {Type: this.TokenTypeDollar, Representation: [b]});
i = (i + 1);
            continue;
        }
        if ((b == 58)) {
tokens = append(tokens, {Type: this.TokenTypeColon, Representation: [b]});
i = (i + 1);
            continue;
        }
        if ((b == 44)) {
tokens = append(tokens, {Type: this.TokenTypeComma, Representation: [b]});
i = (i + 1);
            continue;
        }
...
tokens = append(tokens, {Type: this.TokenTypeNumber, Representation: repr});
            continue;
        }
        if (this.IsAlpha(b)) {
let repr = [];
...
tokens = append(tokens, {Type: this.TokenTypeIdentifier, Representation: repr});
            continue;
        }
i = (i + 1);
    }
...
let hasLabel = false;
        if ((((tokens[i].Type == this.TokenTypeIdentifier) && ((i + 1) < len(tokens))) && (tokens[(i + 1)].Type == this.TokenTypeColon))) {
...
let instr = {OpcodeBytes: this.CopyBytes(tokens[i].Representation), Mode: this.ModeImplied, Operand: 0, LabelBytes: currentLabelBytes, HasLabel: hasLabel};
i = (i + 1);
...
i = (i + 1);
                 ...
                 else                 if (((i < len(tokens)) && (tokens[i].Type == this.TokenTypeNumber))) {
...
i = (i + 1);
                        if (((i < len(tokens)) && (tokens[i].Type == this.TokenTypeIdentifier))) {
...
i = (i + 1);
             ...
             else             if (((tokens[i].Type == this.TokenTypeIdentifier) && this.MatchToken(tokens[i], "A"))) {
...
code = append(code, uint8((((instr.Operand >> 8)) & 0xFF)));
            ...
            if (((instr.Mode == this.ModeAccumulator) || (instr.Mode == this.ModeImplied))) {
